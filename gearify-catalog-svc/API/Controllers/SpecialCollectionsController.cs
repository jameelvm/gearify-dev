...
public class SpecialCollectionsController : ControllerBase
{
    ...
    /// <summary>
    /// Get special product collections for the tenant (Deals, Clearance, etc.)
    ...
    public async Task<IActionResult> GetSpecialCollections()
    {
        try
        {
            var result = await _mediator.Send(new GetSpecialCollectionsQuery());
            return Ok(result);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error fetching special collections");
